...
router.get("/popularListings", async (req, res, next) => {
    try {
        ...
        res.json(await Shoes.find({_id: {$in: items.map((a) => Types.ObjectId(a.sneakerId))}}));
    } catch (e) {
...
function fakeSort(array, idx, fn) {
    ...
    for (let i = 0; i < array.length; ++i) {
        ...
        const partitionPoint = partition(array, lower, upper, (a) => fn(a, itemInCurrentIter) < 0);
        const partitionPoint2 = partition(array, partitionPoint, upper, (a) => fn(a, itemInCurrentIter) === 0);
...
router.post("/recalculatePopularListings", async (req, res, next) => {
    try {
        ...
        for await (const shoesViewed of allShoesViewed) {
            ...
            for (const [shoeId, amountOfViews] of Object.entries(shoesViewedByPerson)) {
        ...
        const sortedShoes = fakeSort(allShoes, 30, (a, b) => shoeStuff[b] - shoeStuff[a]).slice(0, 30);
        await PopularSneakers.deleteMany({});
        await PopularSneakers.insertMany(sortedShoes.map((sneakerId) => ({sneakerId})));
